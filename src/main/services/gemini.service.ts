...
export class GeminiService {
  ...
  async initialize() {
    try {
      ...
      const StoreClass = typeof StoreModule.default === 'function' ? StoreModule.default : (StoreModule as any)
  ...
  async generateContentStream(messages: Array<{ role: string, parts: { text: string }[] }>, onChunk: (chunk: string) => void) {
    ...
    console.log(`[GeminiService] Starting request sequence. Prompt ID: ${userPromptId} (Session: ${this.sessionId}, Tier: ${this.userTier})`)
    ...
    await retryWithBackoff(async () => {
      ...
      console.log(`[GeminiService] Connection established for ${userPromptId}. Consuming stream...`)
      ...
      console.warn(`[GeminiService] RETRY REQUIRED (${attempt}/10). Status: ${error.status || 'unknown'}. Waiting ${waitSecs}s. Error: ${error.message}`)
