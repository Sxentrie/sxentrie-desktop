...
const LOG_DIR = '.logs'
const LOG_FILE = `${LOG_DIR}/dev.log`
...
const preflight = () => {
  ...
  if (!existsSync(LOG_DIR)) {
    // mkdirSync will throw EPERM/EACCES if permissions are wrong. Without
    // this guard, that surfaces as a raw, ugly stack trace instead of a
    // clean diagnostic. The session cannot run without a log directory, so
    // we exit cleanly rather than swallowing the error.
    try {
...
const fingerprint = () => {
  ...
  // capture() returns null on command failure and '' on a clean tree — both
  // are falsy. We need to distinguish between the two so a broken git env
  // doesn't silently display a false "✓ clean" badge.
  ...
  // os.cpus() returns an empty array in some containerized/virtualized
  // environments (certain Docker base images, CI runners). Accessing [0].model
  // directly would throw a TypeError. We guard the length and fall back
  // gracefully, and also guard against a blank model string.
  const cpu = cpus()
  const cpuModel =
...
// A log stream failure (disk full, permissions revoked mid-session) must never
// bring down the dev server. We warn loudly to stderr and continue running.
...
// If spawn itself fails (pnpm not in PATH, binary missing, etc.), Node emits
// an 'error' event on the child object. Without this listener, it becomes an
// unhandled exception that crashes the process with a raw stack trace.
child.on('error', (err) => {
  console.log()
